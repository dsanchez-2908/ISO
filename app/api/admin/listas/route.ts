...
// GET /api/admin/listas?cdNorma=X
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { cdNorma, cdEmpresaConsultora, dsNombreLista, dsDescripcion } = body;

    // Validar campos requeridos
    if (!cdNorma || !cdEmpresaConsultora || !dsNombreLista) {
      return NextResponse.json(
        { success: false, error: 'Campos requeridos: cdNorma, cdEmpresaConsultora y dsNombreLista' },
        { status: 400 }
      );
    }
    ...
    const result = await query(`
      INSERT INTO TD_LISTAS (
        cdEmpresaConsultora, cdNorma, dsNombreLista, dsDescripcion,
